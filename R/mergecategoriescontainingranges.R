...
#' Create factor new factor variables from existing factors whose
#' labels contain numeric ranges, like 18 to 24 or $2,000 to $3,000,
# 'by merging adjacent ranges.
#'
...
#'      is 12, 15, 0, etc. This helps when you wish to 
...
MergeRangeCategories = function(input.data, 
    ...
    # Compute counts. Needs to be updated to handle weights?
    counts = table(raw.data)

    # Try to pull appart labels into numbers and non-numbers
    ...
    if (is.unsorted(label.data$second.value, na.rm = TRUE)) {
        stop("The numbers in the data labels suggest that the ranges are inconsistent or overlapping. ",
            ...
            "referring to months and others referring to years. Please esnure your labels contain consistent units.")
    }
    ...
    if (method[1] == "even.proportions") {
        ...
        stop("Method, '", method[1],"' not recognized. Should be ",
...
extractRangeInformationFromLabel <- function(x, grouping.mark = ",", decimals.mark = ".") {
        ...
        if (length(numbers) + 1 != length(non.numbers)) {
            stop("Chris, you've got the splitting wrong!")
        }
        ...
        all.deliiters.same = all(delimiter.candidates == delimiter.candidates[1])
...
identifyClosedOrOpenBoundariesFromText = function(label) {
    ...
    lower.boundary.open.phrases = c("less than", "under", "fewer than", "lower than", "below", "<", "smaller than")
    lower.boundary.closed.phrases = c( "up to", "<=", "and less", "and under", "and lower", "and below", "and smaller")

    upper.boundary.open.phrases = c("greater than", "over", "more than", "above", ">")
