...
MergeRangeCategories <- function(input.data,
    ...
    if (!is.ranges) {
        StopForUserError("Could not detect sufficient numeric data to merge ranges. When combining ",
    ...
    if (is.unsorted(label.data$second.value, na.rm = TRUE)) {
        StopForUserError("The numbers in the data labels suggest that the ranges are inconsistent or overlapping. ",
